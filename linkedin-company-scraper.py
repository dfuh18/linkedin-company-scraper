...
def maybe_login(driver):
    ...
    print(f"🔐 Logging in as {email[:5]}...@{email.split('@')[1] if '@' in email else 'unknown'}")
    ...
    try:
        ...
        if "challenge" in current_url or "verification" in current_url or "security" in page_source:
            print("⚠️  Security challenge detected!")
            print("🔍 Current URL:", current_url)
            
            if "email" in page_source and "verification" in page_source:
                print("📧 Email verification required")
                input("Please check your email and complete verification, then press Enter to continue...")
            elif "phone" in page_source and "verification" in page_source:
                print("📱 Phone verification required")
                input("Please complete phone verification, then press Enter to continue...")
            else:
                print("🤖 Bot detection / CAPTCHA challenge")
                print("Please manually complete the challenge in the browser window")
                input("After completing the challenge, press Enter to continue...")
            ...
            if "feed" in current_url or "in.linkedin.com" in current_url:
                ...
                print("⚠️  Still on challenge page. Please check the browser.")
        ...
        input("Please manually log in if needed, then press Enter to continue...")
        ...
        try:
            ...
            if id_match:
                ...
                if id_match:
                    ...
                    id_match = re.search(r'"companyUrn":"urn:li:company:(\d+)"', page_source)
                    if id_match:
                        linkedin_company_id = id_match.group(1)
                        ...
                        if id_match:
                            linkedin_company_id = id_match.group(1)
                            print(f"✅ Found company ID (method 4): {linkedin_company_id}")
                        else:
...
def save_to_supabase(company_data):
    ...
    try:
        # Clean the data for Supabase - handle None values and ensure proper types
        clean_data = {}
        for key, value in company_data.items():
            if value is None:
                clean_data[key] = None
            elif key == 'specialties' and isinstance(value, str):
                # Handle specialties as text (not array) since it comes as newline-separated string
                clean_data[key] = value
            elif isinstance(value, list):
                # Convert list to comma-separated string
                clean_data[key] = ', '.join(str(item) for item in value) if value else None
            else:
                clean_data[key] = str(value)
        
        # Ensure scraped_at is in proper timestamp format
        if 'scraped_at' in clean_data:
            clean_data['scraped_at'] = clean_data['scraped_at'].replace('T', ' ').split('.')[0]
        
        result = supabase.table("companies").insert(clean_data).execute()
        print(f"✅ Saved to database: {company_data['name']}")
        return result
    except Exception as e:
        print(f"❌ Error saving to database: {str(e)}")
        print(f"   Data that failed: {clean_data}")
        return None


def scrape_multiple_companies(company_urls: list, delay_range=(2, 5), mode="single_session"):
    """
...
def company_name_to_linkedin_url(company_name):
    ...
    if url_name in special_cases:
        ...
        url_name = re.sub(r'[^\w\s-]', '', url_name)  # Remove special characters except spaces and dashes
        url_name = re.sub(r'\s+', '-', url_name)  # Replace spaces with dashes
        url_name = re.sub(r'-+', '-', url_name)  # Replace multiple dashes with single dash
...
def scrape_companies_with_gemini_verification(filename, max_companies=None, use_gemini=True):
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=3, help='Number of companies to process (default: 3)')
    parser.add_argument('--delay', type=int, default=5, help='Delay between requests in seconds (default: 5)')
    parser.add_argument('--use-gemini', action='store_true', help='Use Gemini for URL verification')
    parser.add_argument('--file', type=str, default='company-list.txt', help='Input file with company names')
    parser.add_argument('--mode', type=str, choices=['single_session', 'per_company'], default='single_session', 
    ...
    try:
        ...
        results = scrape_multiple_companies(company_urls, delay_range=(args.delay, args.delay + 2), mode=mode)
    ...
    if results:
        ...
        if company_ids:
            ...
            print("❌ No company IDs found. Check the company URLs or LinkedIn page structure.")
    else:
